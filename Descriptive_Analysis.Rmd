...
### Table 1: Attendance summary overall
...
### Figure 2: Audience Composition by Month

```{r}
audience_month <- parkevent_long %>%
  group_by(month, audience_type) %>%
...
fig_audience <- plot_ly(
  audience_month,
  x = ~month,
  y = ~prop,
  type = "bar",
  color = ~audience_type,
  colors = c("Adults" = "tomato",
             "Kids" = "seagreen3",
             "Seniors" = "steelblue"),
  hovertemplate = paste(
    "<b>%{x}</b><br>",
  ...
  customdata = ~audience_type
) %>%
  layout(
    barmode = "stack",
    title = "Audience Composition by Month",
...
### Figure 3: Seasonal Attendance Patterns
